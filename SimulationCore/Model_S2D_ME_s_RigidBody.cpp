...
int Model_S2D_ME_s_RigidBody::output(std::fstream &model_file)
{
	...
	mh.y0 = y0;
	mh.yn = yn;
	mh.elem_x_num = elem_x_num;
	mh.elem_y_num = elem_y_num;
	model_file.write(reinterpret_cast<char *>(&mh), sizeof(mh));

	// rigid body
	RigidBodyHeader rbh;
	TriangleMesh &rb_mesh = rigid_body.mesh;
	rbh.node_num = rb_mesh.get_node_num();
	rbh.elem_num = rb_mesh.get_elem_num();
	model_file.write(reinterpret_cast<char *>(&rbh), sizeof(rbh));
	// node coordinates
	...
	model_file.write(reinterpret_cast<char *>(node_coords), sizeof(double)*rbh.node_num);
	delete[] node_coords;
	// element topology
	...
	model_file.write(reinterpret_cast<char *>(elem_indices), sizeof(unsigned long long)*rbh.elem_num);
	delete[] elem_indices;

	// material point object
	MPObjectHeader mph;
	mph.pcl_num = pcl_num;
	model_file.write(reinterpret_cast<char *>(&mph), sizeof(mph));
