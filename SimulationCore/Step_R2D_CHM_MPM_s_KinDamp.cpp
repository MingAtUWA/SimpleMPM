...
Step_R2D_CHM_MPM_s_KinDamp::Step_R2D_CHM_MPM_s_KinDamp() :
	Step(&solve_substep_R2D_CHM_MPM_s_KinDamp),
	model(nullptr),
...
int Step_R2D_CHM_MPM_s_KinDamp::init()
{
	...
	prev_kinetic_energy_s = 0.0;
	prev_kinetic_energy_f = 0.0;

	return 0;
}

int Step_R2D_CHM_MPM_s_KinDamp::finalize() { return 0; }
...
int solve_substep_R2D_CHM_MPM_s_KinDamp(void *_self)
{
	...
	double cur_kinetic_energy_s = 0.0;
	double cur_kinetic_energy_f = 0.0;
	for (size_t i = 0; i < model->pcl_num; i++)
	{
		ppcl = model->pcls + i;
		if (ppcl->is_in_mesh)
		{
			...
			// kinetic energy
			cur_kinetic_energy_s += ppcl->vol * (1.0-ppcl->n) * ppcl->density_s
								 * (ppcl->vx_s * ppcl->vx_s + ppcl->vy_s * ppcl->vy_s);
			cur_kinetic_energy_f += ppcl->vol * ppcl->n * ppcl->density_f
								 * (ppcl->vx_f * ppcl->vx_f + ppcl->vy_f * ppcl->vy_f);
		}
	}

	// Reset all velocity to zero and restart calculation
	// The criteria may need to be modified
	if (cur_kinetic_energy_s < self->prev_kinetic_energy_s
		|| cur_kinetic_energy_f < self->prev_kinetic_energy_f)
	{
		for (size_t i = 0; i < model->pcl_num; i++)
		{
			ppcl = model->pcls + i;
		...
		self->prev_kinetic_energy_s = 0.0;
		self->prev_kinetic_energy_f = 0.0;
		return 0;
	}
	self->prev_kinetic_energy_s = cur_kinetic_energy_s;
	self->prev_kinetic_energy_f = cur_kinetic_energy_f;
