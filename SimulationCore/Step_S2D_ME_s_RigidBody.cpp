...
int Step_S2D_ME_s_RigidBody::finalize() { return 0; }
...
int solve_substep_S2D_ME_s_RigidBody(void *_self)
{
	...
	md.rigid_body.predict_motion_from_ext_force(self.dt);
	...
	for (size_t elem_id = 0; elem_id < md.elem_num; ++elem_id)
	{
		...
		{
			for (Particle_rb *pcl_iter= elem.pcls; pcl_iter;
				 pcl_iter = pcl_iter->next_by_elem)
			{
	...
	double dt_square = self.dt * self.dt;
	...
	for (size_t n_id = 0; n_id < md.node_num; ++n_id)
	{
		...
		if (n.fx_con != 0.0 && n.fy_con != 0.0)
		{
			...
			n.ax += ax_corr;
			n.vx += ax_corr * self.dt;
		}
		if (n.fy_con != 0.0)
		{
			ay_corr = n.fy_con / n.m;
			n.ay += ay_corr;
			n.vy += ay_corr * self.dt;
		}
	}
	...
	md.rigid_body.correct_motion_by_con_force(self.dt);
