...
Model_R2D_CHM_MPM_s::Model_R2D_CHM_MPM_s() :
	Model("R2D_CHM_MPM_s"),
...
void Model_R2D_CHM_MPM_s::set_local_damping(double a_s, double a_f)
{
	alpha_s = a_s;
	alpha_f = a_f;
}

Element_R2D_CHM_MPM_s *Model_R2D_CHM_MPM_s::find_in_which_element(double x, double y)
{
...
Element_R2D_CHM_MPM_s *Model_R2D_CHM_MPM_s::find_in_which_element(
	double x, double y, Element_R2D_CHM_MPM_s *elem)
{
	size_t elem_x_id, elem_y_id;
	...
	if (elem)
	{
		...
		return elems + elem_x_num * elem_y_id + elem_x_id + elem_id_offset;
	}
...
double Model_R2D_CHM_MPM_s::cal_characteristic_length(Element_R2D_CHM_MPM_s *elem)
{
...
void Model_R2D_CHM_MPM_s::cal_shape_function(Particle_R2D_CHM_s *pcl)
{
	...
	pelem = static_cast<Element_R2D_CHM_MPM_s *>(pcl->elem);
...
int Model_R2D_CHM_MPM_s::find_x_index(double x, size_t *index)
{
...
// return index of left side of interval
// note that tail is the last index + 1
int Model_R2D_CHM_MPM_s::find_y_index(double y, size_t *index)
{
