...
Step_R2D_ME_MPM_s_KinDamp::Step_R2D_ME_MPM_s_KinDamp() :
	Step(&solve_substep_R2D_ME_MPM_s_KinDamp),
	model(nullptr),
...
int Step_R2D_ME_MPM_s_KinDamp::finalize() {	return 0; }

int solve_substep_R2D_ME_MPM_s_KinDamp(void *_self)
{
	...
	double E_tmp;
	double cur_kinetic_energy;
	double pcl_vx_tmp, pcl_vy_tmp;
	cur_kinetic_energy = 0.0;
	for (size_t i = 0; i < model->pcl_num; i++)
	{
		ppcl = model->pcls + i;
		if (ppcl->is_in_mesh)
		{
			...
			// cal kinetic energy
			pcl_vx_tmp = ppcl->mmx / ppcl->m;
			pcl_vy_tmp = ppcl->mmy / ppcl->m;
			cur_kinetic_energy += ppcl->m * (pcl_vx_tmp * pcl_vx_tmp + pcl_vy_tmp * pcl_vy_tmp);
		}
	}

	// reset all velocity to zero and restart calculation
	if (cur_kinetic_energy < self->prev_kinetic_energy)
	{
		for (size_t i = 0; i < model->pcl_num; i++)
		{
			ppcl = model->pcls + i;
		...
		self->prev_kinetic_energy = 0.0;
		return 0;
	}
	self->prev_kinetic_energy = cur_kinetic_energy;
