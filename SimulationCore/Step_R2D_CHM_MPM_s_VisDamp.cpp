...
int solve_substep_R2D_CHM_MPM_s_damp(void *_self)
{
	...
	for (size_t i = 0; i < model->pcl_num; i++)
	{
		...
		if (ppcl->is_in_mesh)
		{
			...
			pn1->m_f    += ppcl->N1 * ppcl->n * ppcl->density_f * ppcl->vol;
			...
			pn2->m_f    += ppcl->N2 * ppcl->n * ppcl->density_f * ppcl->vol;
			...
			pn3->m_f    += ppcl->N3 * ppcl->n * ppcl->density_f * ppcl->vol;
			...
			pn4->m_f += ppcl->N4 * ppcl->n * ppcl->density_f * ppcl->vol;
		}
	}
