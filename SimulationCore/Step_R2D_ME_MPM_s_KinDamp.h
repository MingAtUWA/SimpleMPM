#ifndef _STEP_R2D_ME_MPM_S_KINDAMP_H_
#define _STEP_R2D_ME_MPM_S_KINDAMP_H_
...
// Dynamic relaxation method with kinetic damping
// for single object only
class Step_R2D_ME_MPM_s_KinDamp : public Step
{
protected:
	Model_R2D_ME_MPM_s *model;
	
public:
	Step_R2D_ME_MPM_s_KinDamp();
	~Step_R2D_ME_MPM_s_KinDamp();
	...
	// Restart version
	inline void set_prev_step(Step_R2D_ME_MPM_s_KinDamp *prev_step)
	{
...
protected:
	int init(void);
	friend int solve_substep_R2D_ME_MPM_s_KinDamp(void *_self);
	int finalize(void);
